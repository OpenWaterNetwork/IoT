...
class SIM800L():
    ...
    def __init__(self,simOp,ntp):
        self.__ser_gprs = UART(1, baudrate=9600)
        self.simOperator = simOp
        self.ntp_server = ntp
        self.__rtc = RTC()
        time.sleep(1.0)
...
####--- Method of reading the GPRS response to AT commands.
    ...
    def readGPRSdata(self):
        ...
        while (datos_gprs == True):
            ...
            if(self.__ser_gprs.any() > 0):
                gprsdataread= gprsdataread + str(self.__ser_gprs.read())
                readData = True
                scape = False
                chrono.reset()
            elif readData == True:
    ...
    def uart_waiting(self):
        while True:
            if self.__ser_gprs.any():
                break

    def readUDPThread(self):

        self.uart_waiting()
        utime.sleep_ms(50)
        data = self.__ser_gprs.read()
...
###-- Connect to the Internet to initiate communication.
    ...
    def GPRS_init(self):
        ...
        try:
            ...
            self.readGPRSdata()
            self.__ser_gprs.write("AT+SAPBR=3,1,\"APN\",\""+self.simOperator+"\"\r\n")
            self.readGPRSdata()
    ...
    def GPRS_NTP(self):
        ...
        try:
            ...
            if float_retransmit==True:
                ...
                self.readGPRSdata()
                self.__ser_gprs.write("AT+CNTP=\""+self.ntp_server+"\",-20"+"\r\n")   ###-- Ip NTP server
                self.readGPRSdata()
...
###-- Sending data to the server provided, via HTTP get.
    ...
    def UDP_connected(self,mqttserver,port):
        ...
        try:
            ...
            for i in range (0,2):
                ...
                if comp1==False:
                    ...
                    self.__ser_gprs.write("AT+CIPSTART=\"UDP\",\""+mqttserver+"\",\""+str(port)+"\"\r\n")
                    time.sleep(4)
                    data1=self.readGPRSdata()
                    #time.sleep(1)
                    #data2=self.readGPRSdata()

                    comp1="CONNECT OK" in str(data1)

                    if comp1==True:
                        #print("UDP server connected")
                        float_transmit=True
                    else:
                        float_transmit=False
                        self.closeUDP()
                        time.sleep(4)
        ...
        return float_transmit

    def verifyConnect(self):
        self.__ser_gprs.write("AT+CIPSTART=?\r\n")
        time.sleep(2)
        data1=self.readGPRSdata()

    def closeUDP(self):
        self.__ser_gprs.write("AT+CIPSHUT\r\n")
        time.sleep(2)
        dataread=self.readGPRSdata()
        dataOk="SHUT OK" in dataread
        if dataOk==False:
            dataread=self.readGPRSdata()
...
###-- GPRS in sleep mode to reduce power consumption.
    ...
    def GPRS_sleep(self):
        self.__ser_gprs.write("AT+CSCLK=2\r\n")
        gprsdataread=self.readGPRSdata()
        time.sleep(2)

    def GPRS_wakeUp(self):
        self.__ser_gprs.write("AT+CSCLK=0\r\n")
        gprsdataread=self.readGPRSdata()
        time.sleep(1)
